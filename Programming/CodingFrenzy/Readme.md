...
6. 此為題目練習視窗，視窗各區塊的功能說明如下：
    ...
    1. 功能表
    2. 題文/解文：某些課程中的題目詳細內容會顯示在這裡。「程式練習廣場」中的題目，則會在此處顯示解題步驟，可按右上角的 x 按鈕關閉此區塊。
    3. 程式碼輸入
    4. 程式輸出
    5. 建置測試：包含以下各項目。
        * 題目資料：會顯示題目說明(或題目名稱)
...
8. 請依照題目的要求，在程式碼輸入區撰寫程式(按 `Ctrl + Z` 組合鍵可復原前一次的編輯)。寫完程式後，按下「隨機測資」功能，即可進行編譯測試 (輸入的程式碼必須符合題目要求，編譯測試才會通過。)：
