...
2. 此時會顯示「Git Synchronization」對話視窗，請請點選下方的「Pull」按鈕旁的箭頭以開啟下拉選單，然後點選 `Fetch`：

	![](./images/103.png)

3. 當 `Fetch` 完成後，會顯示如下畫面(Ref List 頁籤)：
...
2. 在想要刪除分支(例如 `style`)上按右鍵，點選「Delete branch」：
...
6. 若你想解決衝突，可依下列步驟操作：
	...
	請在發生衝突的檔案上按右鍵，點選 `Edit conflicts`：
...
2. 請在 Stash Message 下方的文字方塊輸入儲藏訊息，然後點選 OK 按鈕:

	![](./images/90.png)

3. 當儲藏建立完成後，會顯示如下視窗，請點選 Close 按鈕關閉視窗：
...
2. 此時會顯示 RefLog 視窗，並顯示 Stash 清單：
...
3. 顯示的檔案變更清單如下圖，請點選 OK 按鈕關閉視窗即可：
...
2. 此時會顯示 RefLog 視窗，並顯示 Stash 清單，請在要移除的儲藏項目上按右鍵，然後點選「Delete Ref...」：

	![](./images/100.png)

3. 此時會顯示如下圖的訊息，請點選 `Delete` 刪除該儲藏項目：

	![](./images/101.png)

4. 下圖是刪除完後的 Stash 清單，請點選 OK 按鈕關閉視窗即可：
...
2. 此時會顯示「Git Synchronization」對話視窗，請請點選下方的「Pull」按鈕旁的箭頭以開啟下拉選單，然後點選 `Fetch & Rebase`：
...
4. 完成時會顯示如下畫面，請點選 `Done` 關閉 `Rebase` 視窗：

	![](./images/108.png)


5. 當回到「Git Synchronization」視窗時，會顯示如下畫面(In Commits 頁籤)：
...
2. 在想要Cherry Pick的分支(例如 `style`)上按右鍵，點選「Show log」：
...
4. 此時會顯示 Cherry Pick 視窗，請點選 `Continue` 按鈕進行 Cherry Pick：

	![](./images/113.png)

	Cherry Pick 進行時，可能會遇到衝突，請解決衝突，然後點選 `Commit` 按鈕：

	![](./images/114.png)

	當完成 Cherry Pick 時，會顯示如下畫面：

	![](./images/115.png)

	請點選 `Close` 按鈕關閉 Cherry Pick 視窗即可。
